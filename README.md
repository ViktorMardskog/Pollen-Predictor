...
We have trained one model per pollen type have have measured performance for each model at different time horizons. XGBoost was used to model the pollen levels. Here are the results for each model:

(Note that R squared is for predicting levels for day t with data from t-1 (same as for MSE_h1).)
